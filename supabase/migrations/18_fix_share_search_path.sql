-- Fix search_path to include extensions schema for sharing functions
-- This allows access to gen_random_bytes (from pgcrypto) when generating sharing tokens.

-- 1. Redefine create_bookmark_share
...
-- Since the body is very simple, let's copy the actual logic:
CREATE OR REPLACE FUNCTION create_bookmark_share(
    resource_id UUID,
    expires_at TIMESTAMPTZ DEFAULT NULL,
    permission_level TEXT DEFAULT 'view'
...
-- 2. Redefine create_folder_share
